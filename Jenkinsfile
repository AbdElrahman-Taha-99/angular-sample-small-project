pipeline {
    ...
    stages {
        ...
        stage('Install Node.js') {
            steps {
                sh 'sudo -u jenkins bash -c "curl -fsSL https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.4/install.sh | bash"'
                sh 'sudo chown -R jenkins:jenkins /var/lib/jenkins/.nvm'               
                sh 'source ~/.bashrc'
                sh 'nvm install --lts'
                sh 'nvm use --lts'
